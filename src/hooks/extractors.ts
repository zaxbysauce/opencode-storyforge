/**
 * Writer Swarm File Extractors
 *
 * Pure parsing functions for extracting structured data from .writer/ files.
 * Uses markdown AST parsing instead of regex.
 * Used by system-enhancer hook.
 */
...
function evictEntriesIfNeeded(): void {
	...
	// Then, check size limit (evict oldest entries)
	while (
...
export function extractCurrentPhase(planContent: string): string | null {
	...
	visit(tree, (node) => {
		...
		if (node.type === 'heading') {
			...
			if (text === 'Workflow Status' || text === 'Status') {
				currentSection = text;
